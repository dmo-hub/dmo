...
- **Lint/format:** [Ruff](https://docs.astral.sh/ruff/) — config in [`pyproject.toml`](pyproject.toml).
  Advisory in CI (warn-first); run locally with `ruff check .` and `ruff format .`.
...
|---|---|---|
| [`validate.yml`](.github/workflows/validate.yml) | push / PR | JSON + HTML + inline-JS checks + Ruff (advisory). Writes a run summary to the Actions UI. Gates Pages. |
